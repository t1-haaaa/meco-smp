...
regions = glob.glob('arena/region/r.*.*.mca')
rx_min = min(int(p.split('/')[-1].split('.')[1]) for p in regions)
rx_max = max(int(p.split('/')[-1].split('.')[1]) for p in regions)
rz_min = min(int(p.split('/')[-1].split('.')[2]) for p in regions)
rz_max = max(int(p.split('/')[-1].split('.')[2]) for p in regions)
print("regions:", len(regions))
print("region x %d..%d, z %d..%d" % (rx_min, rx_max, rz_min, rz_max))
print("world x %d..%d, z %d..%d" % (rx_min * 512, (rx_max + 1) * 512 - 1, rz_min * 512, (rz_max + 1) * 512 - 1))

cache = {}
for p in regions:
    rx = int(p.split('/')[-1].split('.')[1]); rz = int(p.split('/')[-1].split('.')[2])
    with open(p, 'rb') as f:
        tbl = f.read(4096)
    for lcz in range(32):
        for lcx in range(32):
            ...
            try:
                cache[(cx, cz)] = load_chunk(p, cx, cz)
            except Exception:
                pass
print("chunks loaded:", len(cache))

wx0, wx1 = rx_min * 512, (rx_max + 1) * 512
wz0, wz1 = rz_min * 512, (rz_max + 1) * 512
...
for yl in (64, 65, 70, 80):
    print("\n===== OVERVIEW y=%d step=8 =====" % yl)
    for wz in range(wz0, wz1, 8):
        print("%6d | %s" % (wz, ''.join(char(sample(wx, wz, yl)) for wx in range(wx0, wx1, 8))))